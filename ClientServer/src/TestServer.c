...
int main(int argc, char **argv){
	...
	int MaxDataPerPacket = 256;
	int DATA_Position = 0; 	// Position of the packet in the line  - 0 to 255
	...
	int *UDPBuffer = (int *)malloc(SizeOfPacket * sizeof(int));

	for (i=0 ; i<NbOfPacket ; i++) { // iteration sur l'ensemble des packets du buffer
		UDPBuffer[0] = DATA_Image;
		UDPBuffer[1] = DATA_Line;
		UDPBuffer[2] = i;
		for (j=0; j < MaxPointsPerPacket ; j++){ // iteration par points du packet
			UDPBuffer[3+j] = ibuff[i*MaxPointsPerPacket+j];
			//printf("%dx%d \t %d\n",i,j, UDPBuffer[3+j]);
		}
		CheckUDP = sendto(s,UDPBuffer,SizeOfPacket,0,(struct sockaddr*) &si_other,slen);
